...
public class EffWalls extends Effect {
    ...
    protected void execute(Event event) {
        ...
        RunnableUtils.run(Runnables.getWallsRunnable(wrapper, pattern));
    }

    @SuppressWarnings("NullableProblems")
    @Override
    public String toString(@Nullable Event event, boolean debug) {
        return "create walls around " + wrapper.toString(event, debug) + " with pattern " + prePattern.toString(event, debug);
    }
}
